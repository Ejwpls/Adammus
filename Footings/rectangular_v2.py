...
class FoundationCalculator:
    ...
    def calculate(self, sizes: np.ndarray) -> np.ndarray:
        ...
        to_calculate = list(self.calculations.items())

        while to_calculate:
            name, calc = to_calculate.pop(0)
            args = calc.func.__code__.co_varnames
            missing_args = [arg for arg in args if arg not in results]

            if not missing_args:
                arg_values = [results[arg] for arg in args]
                results[name] = calc.func(*arg_values)
                calculated.add(name)
            else:
                print(f"Skipping {name}. Missing arguments: {', '.join(missing_args)}")
                to_calculate.append((name, calc))
...
class FoundationSizer:
    ...
    def generate_foundation_sizes(self) -> np.ndarray:
        ...
        print("Starting generation of valid combinations...")
        valid_combinations = self.generate_valid_combinations(FTG_LENGTHS, FTG_WIDTHS, FTG_DPTHS, FTG_CONC_STRENGTHS, FTG_REO_SIZES, FTG_REO_CTS, FTG_REO_SIZES, FTG_REO_CTS)
        ...
        structured_array = np.array(valid_combinations, dtype=FOUNDATION_SIZE_DTYPE)
    ...
    def is_valid_lw(l, w, col_length, col_width):
        return ((col_length <= col_width and l <= w) or 
                (col_length > col_width and l >= w)) and not (1 < l/w <= 1.1 or 0.9 <= l/w < 1)

    @jit(nopython=True)
    def generate_valid_combinations(self, ftg_lengths, ftg_widths, ftg_depths, ftg_conc_strengths, ftg_reo_sizes, ftg_reo_cts, _, __):
        valid_combinations = []
        for l in ftg_lengths:
            for w in ftg_widths:
                if self.is_valid_lw(l, w, self.params.COLUMN_LENGTH, self.params.COLUMN_WIDTH):
                    for d in ftg_depths:
                        for cs in ftg_conc_strengths:
                            for rl in ftg_reo_sizes:
                                for rcl in ftg_reo_cts:
                                    for rw in ftg_reo_sizes:
                                        for rcw in ftg_reo_cts:
                                            if (self.check_reinforcement_constraint(rl, rcl, d, self.params.FTG_COVER) and
                                                self.check_reinforcement_constraint(rw, rcw, d, self.params.FTG_COVER)):
